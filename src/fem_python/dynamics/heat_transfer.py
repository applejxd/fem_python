...
class HeatTransfer:
    def __init__(
        ...
        self.A, self.B = self.coeff(diffusivity=diffusivity, theta=theta)
...
class AnimCreator:
    ...
    def create(self, save: bool = False):
        anim = FuncAnimation(
            fig=self.fig,
            func=self.update,
            frames=self.simulator.evolve(t=0.0, u=self.simulator.u_init),
            repeat=False,
            # 10 ms おきにプロット
            interval=10,
        )
