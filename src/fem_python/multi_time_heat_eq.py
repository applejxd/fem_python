...
alpha = 0.25
t_start = nm.array([0.0, 20.0, 40.0])  # times when heating starts (seconds)
t_stop = nm.array([10.0, 30.0, 50.0])  # times when heating stops (seconds)

T0 = 25.0  # °C
h = 50.0  # W/m2/K
mm = 1e-3

url = r"https://github.com/sfepy/sfepy/raw/master/meshes/3d/multi_material_cylinder_plate.vtk"
filename_mesh = utils.download_file(url)

materials = {
    "powder": ({"lam": 0.16, "rho_cp": 1330.0 * 650.0},),
    "cylinder": (
        {"lam": 153.0, "rho_cp": 2760.0 * 935.0, "lam_vec": 173.0 * nm.eye(3)},
    ),
    "plate": ({"lam": 153.0, "rho_cp": 2660.0 * 927.0, "lam_vec": 153.0 * nm.eye(3)},),
    "heat_flux_defined_by_func": "get_flux_value",
    "heat_loss": ({"h_bot": -h, "T_bot_inf": T0, "h_top": -h, "T_top_inf": T0},),
}
...
def gen_probe():
    ...
    p0, p1 = nm.array([0.0, 0.0, -10.0 * mm]), nm.array([0.0, 0.0, 15.0 * mm])
...
def post_process_hook(out, pb, state, extend=False):
    ...
    if ts.step == ts.n_step - 1:
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(pb.output_dir, "heat_probe_time_evolution.png"),
            bbox_inches="tight",
        )
    return out
